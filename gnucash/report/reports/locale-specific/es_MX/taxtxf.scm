...
;; Converts non-MXN transaction amounts based on transaction data or, if
...
;; Add support for taxpayer types other than Ind
;;
...
(define-module (gnucash reports locale-specific es_MX taxtxf))
(use-modules (gnucash engine))
(use-modules (gnucash utilities))
(use-modules (gnucash core-utils)) ; for gnc:version and (G_ ...)
(use-modules (gnucash app-utils))
(use-modules (gnucash locale es_MX tax))
...
;                        "CURRENCY"
;                        "MXN"))
;; this comes back as PEN??? rather than MXN; need to define further down to get
;; to work - weird
...
(define (gnc:display-report-list-item item port warn-msg)
  (cond
   ...
   ((list? item) (map (lambda (item)
                      ...
                      item))
   (else (gnc:warn warn-msg item (N_ " is the wrong type.")))))
       ...
       (append (list (gnc:make-html-table-header-cell/markup
               ...
               (list (gnc:make-html-table-header-cell/markup
                         ...
                         (if (qof-book-use-split-action-for-num-field
                             ...
                             (N_ "Num"))))
               (list (gnc:make-html-table-header-cell/markup
                          "column-heading-center" (N_ "Description")))
               (list (gnc:make-html-table-header-cell/markup
                          "column-heading-center"
                         (if suppress-action-memo?
                             ...
                             (if (qof-book-use-split-action-for-num-field
                                                         (gnc-get-current-book))
                                 (string-append (N_ "Notes") "/" (N_ "T-Num") ":" (N_ "Memo"))
                                 (string-append (N_ "Notes") "/" (N_ "Action") ":" (N_ "Memo"))))))
               (list (gnc:make-html-table-header-cell/markup
                          "column-heading-center"
                         (if format4?
                             (N_ "Capital Gains Data")
                             (N_ "Transfer To/From Account(s)"))))
               (list (gnc:make-html-table-header-cell/markup
                              "column-heading-center" (N_ "Amount")))
       )
  )
)
  ...
  (let ((description (gnc:make-html-text))
       ...
       (if (or tax_code? transaction-details?)
           (gnc:html-text-append! description (gnc:html-markup-b
              (string-append "&nbsp; &nbsp; &nbsp; &nbsp;"
                             (if end-bal-text end-bal-text (N_ "Total for ")))))
           (if (not tax_code?)
               (gnc:html-text-append! description (gnc:html-markup-b
                  "&nbsp; &nbsp; &nbsp; &nbsp;"))
           )
       ...
       (let ((description-cell (if (or tax_code? transaction-details?)
            ...
            (if format4?
                (let* ((total-cell (gnc:make-html-table-cell/markup
                                    ...
                                    (N_ "Capital Gains Totals: Sales Amount: ")))
                       (combined-cell (gnc:make-html-table-cell))
                      )
                      (gnc:html-table-cell-append-objects!
                         total-cell
                         cap-gain-sales-total
                         (string-append " " (N_ "Cost") "/" (N_ "Basis") ": " (N_ "Not Available"))
                      )
  ...
  (let* ((print-info (gnc-account-print-info account #f))
    ...
    (if (and txf?
        ...
        (let* ((date-str (if date
               ...
               (value-name (cond
                             ((string=? tax-entity-type (N_ "Ind"))
                               (if (equal? (N_ "ReinvD") action)
                                   (string-append
                                     (xaccPrintAmount
          ...
          ;; fields by format are as follows, for Ind:
          ...
          ;;
          ;; For Ind, support only formats 1, 3 and 6 (based on Q98) and
...
(define (process-currency-conversion split
                                     ...
                                     print-info
                                     neg?)
;; called if account-commodity does not equal MXN-currency or if
;;    trans-currency not equal MXN-currency
;; if trans-currency = MXN-currency and account-commodity does not equal
;;    MXN-currency, use split value & transaction rate
;; if account-commodity = MXN-currency and trans-currency does not equal
;;    MXN-currency, use split amount & inverse of transaction rate
;; if neither trans-currency nor account-commodity = MXN-currency,
...
;;   a list
  ...
  (let*
    ((splt-value (if (and split
                          (or (gnc-commodity-equiv trans-currency MXN-currency)
     ...
     (amount (if (gnc-commodity-equiv trans-currency MXN-currency)
                 splt-value
     ...
     (conversion-text (if missing-pricedb-entry?
                          (string-append
                            "(" (N_ "Missing priceDB entry to convert") " "
                            (gnc-commodity-get-mnemonic account-commodity)
                            " "
                            converted-qty
                            (N_ " to MXN. Go to the Tools->Price Editor dialog to enter prices. Set to zero for this report")
                            ".)"
                          )
                          (string-append
                            (N_ "(Converted ")
                            ...
                            (if
                                ...
                                (string-append " @ " (N_ "PriceDB lookup rate of") " ")
                                (string-append
                                   " @ " (N_ "transaction split rate of") " "
                                   (xaccPrintAmount
...
(define (process-transaction-multi-transfer-detail split parent
            MXN-currency full-names? trans-date trans-currency acct-type
  ...
  (let* ((all-tran-splits (xaccTransGetSplitList parent))
         ...
         (output '())
         (trans-rpt-currency-total (gnc-numeric-zero)) ;;for MXN-currency
         (cap-gains-detail-table (gnc:make-html-table))
         (trans-sub-heading-table (gnc:make-html-table))
         (trans-sub-table (gnc:make-html-table))
         (trans-cap-gain-sales-MXN-total (gnc-numeric-zero))
         (trans-cap-gain-basis-MXN-total (gnc-numeric-zero))
        )
        (if (= 4 format_type)
            (begin
               (gnc:html-table-set-style! cap-gains-detail-table "table"
               ...
               (gnc:html-table-append-row! cap-gains-detail-table
                      (append (list (gnc:make-html-table-cell/markup
                              ...
                              (list (gnc:make-html-table-cell/markup
                                           "column-heading-center"
                                           (string-append (N_ "Cost") "/" (N_ "Basis"))))
                      )
               )
                 ...
                 (let* ((split-acct (xaccSplitGetAccount tran-split))
                       ...
                       (if (and (= 4 format_type) (gnc-numeric-negative-p
                           ...
                           (begin
                              ...
                              (set! trans-cap-gain-sales-MXN-total
                                          (gnc-numeric-add-fixed
                                              trans-cap-gain-sales-MXN-total
                                              (gnc-numeric-neg splt-print-amnt))
                              )
;                              (set! trans-cap-gain-basis-MXN-total #f)
                           )
                       )
        ...
        (if (not (gnc-numeric-equal print-amnt trans-rpt-currency-total))
            (let* ((conversion-text (gnc:make-html-text))
                   (conversion-text-content
                                 (N_ "Multiple currency conversion differences"))
        ...
        (list trans-cap-gain-sales-MXN-total
              trans-cap-gain-basis-MXN-total
              output)
  ) ;; end of let*
)
...
(define (process-account-transaction-detail table account split-list
                ...
                tax-mode? show-TXF-data? MXN-currency account-type
  ...
  (let*
    ((account-commodity (xaccAccountGetCommodity account))
     ...
     (account-MXN-total (gnc-numeric-zero))
                      ...
                      (if show-TXF-data?
                          (let* ((pns (if (or (eq? 'parent code-pns)
                                          ...
                                          (if (eq? 'parent payer-src)
                                              (N_ "Name Source is Parent")
                                              (N_ "Name Source is Current"))
                                          ""))
                                 (line (if (and (= format_type 3)
                                                (or (eq? code-pns 'parent)
                                                    (eq? code-pns 'current)))
                                           (string-append (N_ "Item") " "
                                ...
                                (if (eq? pns "")
                                    (if (eq? line "")
                                        ...
                                        (string-append
                                          " ("
                                          (N_ "TXF Parameter: ") ": " line ")"))
                                    (if (eq? line "")
                                        (string-append
                                          " (" (N_ "TXF Parameter") ": " pns ")")
                                        (string-append
                                          " (" (N_ "TXF Parameters") ": " pns ", "
                                          line ")")))
                          )
                          "")))
          ...
          (if (or (not (gnc-numeric-zero-p beg-bal-acct-curr))
              ...
              (let* ((curr-conv-note "")
                     ...
                     (curr-conv-data (if (gnc-commodity-equiv
                                                 account-commodity MXN-currency)
                                         curr-conv-data
                                         (process-currency-conversion
                                           #f
                                           MXN-currency
                    ...
                    (set! account-MXN-total (gnc-numeric-add-fixed
                                                  account-MXN-total print-amnt))
              );; end of let*
          );; end of if
        );; end of begin
        #f
           ...
           (let* ((parent (xaccSplitGetParent split))
                 ...
                 (if (or (and transaction-details? tax-mode?
                     ...
                     (let ((cap-gain-data
                          ...
                          (set! account-cap-gain-sales-MXN-total
                                          (gnc-numeric-add-fixed
                                              account-cap-gain-sales-MXN-total
                                              (car cap-gain-data))
                          )
                          (set! account-cap-gain-basis-MXN-total
                                          (gnc-numeric-add-fixed
                                              account-cap-gain-basis-MXN-total
                                              (cadr cap-gain-data))
                          )
    ...
    (if tax-mode?
        (let* ((amnt-acct-curr (xaccPrintAmount account-commodity-total
                                                print-info))
               (account-total-amount (xaccPrintAmount account-MXN-total
                                                print-info))
              ...
              (render-total-row table
                                ...
                                (if (gnc-numeric-negative-p account-MXN-total)
                                    #t
                                    #f)
                                (if (= format_type 4)
                                    #t
    ...
    (list account-MXN-total
          output
          (if (or (and (gnc-numeric-positive-p account-MXN-total)
                       (gnc-numeric-positive-p account-commodity-total-as-dr))
                  (and (gnc-numeric-negative-p account-MXN-total)
                       (gnc-numeric-negative-p account-commodity-total-as-dr)))
              account-MXN-total
              (gnc-numeric-neg account-MXN-total))
          account-cap-gain-sales-MXN-total
          account-cap-gain-basis-MXN-total
    )
  ) ;;end of let*
            ...
            (let* ((account-name (gnc-account-get-full-name account))
              ...
              (if (not (string-null? tax-code))
                (if (not (or (null? tax-entity-type)
                             ...
                             (string=? tax-entity-type (N_ "Other"))))
                  (if tax-related
...
;                            );; end if
                      ...
                      (begin
                        (set! txf-invalid-alist (assoc-set!
                                 txf-invalid-alist
                                 tax-code
                                 (list (N_ "Set as tax-related, but assigned tax code no longer valid for tax year")
                     ...
                     (begin
                         (set! txf-invalid-alist (assoc-set!
                                 txf-invalid-alist
                                 tax-code
                                 (list (N_ "Set as tax-related, tax code assigned for different tax entity type")
                                       account-name form account)))
                        selected-accounts-sorted-by-form-line-acct)
                    )
                    (begin ;; not tax related
                      (if (or (not (eqv? form #f))
                          ...
                          (set! txf-invalid-alist (assoc-set!
                                   txf-invalid-alist
                                   tax-code
                                   (list (N_ "Set as not tax-related, but tax code assigned")
                                         account-name form account)))
                          (set! txf-invalid-alist (assoc-set!
                                   txf-invalid-alist
                                   tax-code
                                   (list (N_ "Set as not tax-related, tax code assigned for different tax entity type")
                                         account-name form account)))
                      )
                    selected-accounts-sorted-by-form-line-acct)
                  )
                    ...
                    (if tax-related
                      (begin
                        (set! txf-invalid-alist (assoc-set!
                               txf-invalid-alist
                               (N_ "None")
                               (list (N_ "Set as tax-related, no tax code assigned")
                                     account-name form account)))
                         selected-accounts-sorted-by-form-line-acct)
  ...
  (let* ((from-value (gnc:date-option-absolute-time
                      (get-option gnc:pagename-general (N_ "From"))))
         (to-value (gnc:time64-end-day-time
                    (gnc:date-option-absolute-time
                     (get-option gnc:pagename-general (N_ "To")))))
         (alt-period (get-option gnc:pagename-general (N_ "Alternate Period")))
         (selected-style-sheet (get-option gnc:pagename-general (N_ "Stylesheet")))
         (suppress-0? (get-option gnc:pagename-display
                                 (N_ "Suppress $0.00 values")))
         (full-names? (not (get-option gnc:pagename-display
                                 (N_ "Do not print full account names"))))
         (split-details? (get-option gnc:pagename-display
                                 (N_ "Print all Transfer To/From Accounts")))
         (show-TXF-data? (get-option gnc:pagename-display
                                 (N_ "Print TXF export parameters")))
         (transaction-details? (not (get-option gnc:pagename-display
                                 (N_ "Do not print transaction detail"))))
         (no-special-dates? (get-option gnc:pagename-display
                                 (N_ "Do not use special date processing")))
         (suppress-action-memo? (if (gnc:lookup-option
                                        (gnc:report-options report-obj)
                                            gnc:pagename-display
                                            (N_ "Do not print Action:Memo data"))
                                    (get-option gnc:pagename-display
                                     (N_ "Do not print Action:Memo data"))
                                    (get-option gnc:pagename-display
                                     (N_ "Do not print T-Num:Memo data"))))
         (shade-alternate-transactions? #t)
         (currency-conversion-date (get-option gnc:pagename-display
                                 (N_ "Currency conversion date")))
         (user-sel-accnts (get-option gnc:pagename-accounts
                                 (N_ "Select Accounts (none = all)")))
...
;; subtracted in setting from-value?
             ...
             (if (or (and (or (eq? account-type ACCT-TYPE-INCOME)
                 ...
                 (begin
                    ...
                    (let* ((tran-output (process-account-transaction-detail
                           ...
                           (account-MXN-total-as-dr (caddr tran-output))
                          )
                          (set! account-MXN-total (car tran-output))
                          (set! account-cap-gain-sales-MXN-total
                                                    (cadddr tran-output))
                          (set! account-cap-gain-basis-MXN-total
    ...
    (let ((from-date  (gnc-print-time64 from-value "%Y-%b-%d"))
          ...
          (form-sched-line-MXN-total (gnc-numeric-zero))
          (form-sched-line-cap-gain-sales-MXN-total (gnc-numeric-zero))
          (form-sched-line-cap-gain-basis-MXN-total (gnc-numeric-zero))
          (tax-code-sub-item-MXN-total (gnc-numeric-zero))
          (tax-code-sub-item-MXN-total-as-dr (gnc-numeric-zero))
          (tax-code-MXN-total (gnc-numeric-zero))
          (tax-code-cap-gain-sales-MXN-total (gnc-numeric-zero))
          (tax-code-cap-gain-basis-MXN-total (gnc-numeric-zero))
          (tax-code-MXN-total-as-dr (gnc-numeric-zero))
          (saved-tax-code-text "")
            ...
            (let* ((current-form-schedule (caar form-line-acct))
                          ...
                          (begin
                            (if tax-mode?
                                ...
                                (if (gnc-numeric-zero-p
                                              tax-code-sub-item-MXN-total-as-dr)
                                    #t ;; do nothing
                                    (begin
                                      (set! output
                                            (list (render-txf-account
                                              ...
                                              prior-account-copy
                                              tax-entity-type #f)))
                                      (set! tax-code-MXN-total (gnc-numeric-zero))
                                      (set! tax-code-MXN-total-as-dr
                                                             (gnc-numeric-zero))
                            ...
                            (set! tax-code-sub-item-MXN-total
                                                             (gnc-numeric-zero))
                            (set! tax-code-sub-item-MXN-total-as-dr
                                                             (gnc-numeric-zero))
                          )
                          #f ;; else do nothing
                      )
                  ...
                  (if (string=? prior-tax-code "")
                      ...
                      (if tax-mode?
                             ...
                             (if (and suppress-0? (gnc-numeric-zero-p
                                 ...
                                 (let* ((print-info (gnc-account-print-info
                                        ...
                                        (tax-code-cap-gain-sales-total-amount
                                               (xaccPrintAmount
                                                     tax-code-cap-gain-sales-MXN-total
                                                     print-info))
                                        (tax-code-cap-gain-basis-total-amount
                                               (xaccPrintAmount
                                                     tax-code-cap-gain-basis-MXN-total
                                                     print-info))
                                       )
                                       ...
                                       (set! tax-code-MXN-total
                                                             (gnc-numeric-zero))
                                       (set! tax-code-cap-gain-sales-MXN-total
                                                             (gnc-numeric-zero))
                                       (set! tax-code-cap-gain-basis-MXN-total
                                                             (gnc-numeric-zero))
                                       (set! tax-code-MXN-total-as-dr
                                                             (gnc-numeric-zero))
                                       (set! tax-code-sub-item-MXN-total
                         ...
                         ;; txf output processing
                         (if (gnc-numeric-zero-p tax-code-MXN-total-as-dr)
                             #t ;; do nothing
                  ...
                  (if (string=? prior-form-sched-line "")
                      (set! form-sched-line-MXN-total (gnc-numeric-zero))
                      (if tax-mode?
                         ;; printed report processing
                             ...
                             (if (and suppress-0? (gnc-numeric-zero-p
                                 ...
                                 (let* ((print-info (gnc-account-print-info
                                        ...
                                        (form-sched-line-cap-gain-sales-total-amount
                                          (xaccPrintAmount
                                             form-sched-line-cap-gain-sales-MXN-total
                                             print-info))
                                       ...
                                       (set! form-sched-line-MXN-total
                                                             (gnc-numeric-zero))
                                       (set! form-sched-line-cap-gain-sales-MXN-total
                                                             (gnc-numeric-zero))
                                       (set! form-sched-line-cap-gain-basis-MXN-total
                                                             (gnc-numeric-zero))
                                       (set! txf-l-count 0)
                                 )
                             )
                  ...
                  (let* ((account-output (handle-account
                                                 ...
                                                 acct-full-name
                                                 copy))
                         (account-MXN-total-as-dr (caddr account-output))
                        ...
                        (set! tax-code-MXN-total (gnc-numeric-add-fixed
                                                          tax-code-MXN-total
                                                          (car account-output)))
                        (set! tax-code-cap-gain-sales-MXN-total
                              (gnc-numeric-add-fixed
                        ...
                        (set! tax-code-MXN-total-as-dr (gnc-numeric-add-fixed
                                                       tax-code-MXN-total-as-dr
                                                       account-MXN-total-as-dr))
                        (set! tax-code-sub-item-MXN-total (gnc-numeric-add-fixed
                                                     tax-code-sub-item-MXN-total
                                                          (car account-output)))
                        (set! tax-code-sub-item-MXN-total-as-dr
                                         (gnc-numeric-add-fixed
                                             tax-code-sub-item-MXN-total-as-dr
                                                       account-MXN-total-as-dr))
                        (set! form-sched-line-MXN-total (gnc-numeric-add-fixed
                                                       form-sched-line-MXN-total
                                                          (car account-output)))
                        (set! form-sched-line-cap-gain-sales-MXN-total
                              (gnc-numeric-add-fixed
                        ...
                        (set! form-sched-line-cap-gain-basis-MXN-total
                              (gnc-numeric-add-fixed
                                       form-sched-line-cap-gain-basis-MXN-total
                                       (car (cddddr account-output))))
                        (set! need-form-line-acct-header? #f)
                        (set! need-form-schedule-header? #f)
                        (set! need-tax-code-header? #f)
      ...
      (if (not tax-mode?) ; Do Txf mode
                   ...
                   (lambda (port)
                     (gnc:display-report-list-item
                      (list
                       ...
                       (if (or (zero? tax-code-MXN-total) (not prior-account))
                           '()
                           (render-txf-account
                            prior-account
                            (if (= 4 (get-acct-txf-info
          ...
          (begin  ; else do tax report
                    ...
                    (gnc:html-markup
                     ...
                     (gnc:html-markup/format
                      (string-append (if (and (gnc-get-current-book-tax-name)
                                         ...
                                         (G_ "Tax Name: ~a<br/>")
                                         "~a")
                          (G_ "Period from ~a to ~a<br/>")
                          (G_ "Tax Year ~a<br/>")
                          (G_ "Tax Entity Type: ~a<br/>")
                          (G_ "All amounts in MXN unless otherwise noted"))
                 ...
                 (begin
                   (gnc:html-document-add-object!
                    doc (gnc:make-html-text
                          (gnc:html-markup-p
                           (gnc:html-markup/format
                      (G_ "<br/>The following Account(s) have errors with their Income Tax code assignments (use 'Edit->Tax Report Options' to correct):")))))
                 ...
                 (if (and (or (eqv? (get-acct-txf-info 'pns
                     ...
                     (if (and suppress-0? (gnc-numeric-zero-p
                         ...
                         (let* ((print-info (gnc-account-print-info
                                                              prior-account #f))
                                (tax-code-sub-item-total-amount
                                   (xaccPrintAmount tax-code-sub-item-MXN-total
                                                                    print-info))
                               )
                               (render-total-row
                                           table
                                           ...
                                           (string-append
                                              (if (string=? ""
                                                          prior-form-sched-line)
                                                  (string-append (G_ "Line (Code)") ": ")
                                                  "")
                                              saved-tax-code-text
                                              (string-append ", " (G_ "Item") " ")
                                              (number->string txf-l-count)
                                              ": "
                                              txf-last-payer
                                              " "
             ...
             (if (or (and suppress-0? (gnc-numeric-zero-p tax-code-MXN-total))
                     (null? selected-accounts)
                     (not prior-account))
                 #t ;; do nothing
             ...
             (if (not (string=? prior-form-sched-line ""))
                 (if (or (and suppress-0?
                                 (gnc-numeric-zero-p form-sched-line-MXN-total))
                         (null? selected-accounts)
                         (not prior-account)
                     )
                     #t ;; do nothing
                     (let* ((print-info (gnc-account-print-info prior-account
                            ...
                            (form-sched-line-total-amount
                                      (xaccPrintAmount
                                          form-sched-line-MXN-total print-info))
                            (form-sched-line-cap-gain-sales-total-amount
                                      (xaccPrintAmount
                                        form-sched-line-cap-gain-sales-MXN-total
                                                                    print-info))
                  ...
                  (gnc:make-html-text
                   (gnc:html-markup-p
                     (if (or (null? (gnc-get-current-book-tax-type))
                             ...
                             (string=? (gnc-get-current-book-tax-type) (G_ "Other")))
                       (G_ "The Income Tax Report is only available for valid Income Tax Entity Types. Go to the Edit->Tax Report Options dialog to change your Income Tax Entity Type selection and set up tax-related accounts.")
                       (G_ "No Tax Related accounts were found with your account selection. Change your selection or go to the Edit->Tax Report Options dialog to set up tax-related accounts.")))))
                         ...
                         (gnc:html-markup/format
                          (string-append
                             ...
                             (if (not (null? user-sel-accnts))
                                 (G_ "Subset of accounts")
                                 (G_ "No accounts (none = all accounts)"))
                             (if suppress-0?
                                 (G_ "Suppress $0.00 valued Tax Codes")
                                 (G_ "Do not suppress $0.00 valued Tax Codes"))
                             (if full-names?
                                 (G_ "Display full account names")
                                 (G_ "Do not display full account names"))
                             (if split-details?
                                 (G_ "Display all Transfer To/From Accounts")
                                 (G_ "Do not display all Transfer To/From Accounts"))
                             (if show-TXF-data?
                                 (G_ "Print TXF export parameters")
                                 (G_ "Do not Print TXF export parameters"))
                             (if suppress-action-memo?
                                 (G_ "Do not display Action:Memo data")
                                 (G_ "Display Action:Memo data"))
                             (if transaction-details?
                                 (G_ "Display transactions for selected accounts")
                                 (G_ "Do not display transactions for selected accounts"))
                             (if no-special-dates?
                                 (G_ "Do not include transactions outside of selected dates")
                                 (G_ "Include some transactions outside of selected dates"))
                             (if (equal? currency-conversion-date
                                         'conv-to-tran-date)
                                 (G_ "PriceDB lookups nearest to transaction date")
                                 (G_ "PriceDB lookups nearest to report end date"))
                          )
                        ))))
